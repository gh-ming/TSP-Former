...
class TSP_Former(nn.Module):
    ...
    def forward(self, X, return_feature=False):
        ...
        x,weights = self.psae(x,tsp[:,0]) # b,h,w,t,64
        check("pse",x)
        # temporal embedding
        ...
        if return_feature:
            ...
            x = self.mlp_head(x.reshape(-1, self.dim)).reshape(B, self.num_classes)
            x = x + x_ndvi # [B,K]
            return x, features
...
if __name__ == '__main__':
    ...
    model = TSP_Former(num_classes=2, d_model=128, n_head=4, depth=1, d_inner=64, dropout=0.2, scale_dim=4)
