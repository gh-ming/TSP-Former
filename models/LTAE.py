...
class LTAE(nn.Module):
    def __init__(self, input_dim=10, num_classes=9, d_model=256, n_head=16,
        ...
        # self.mlp_dim = [input_dim, 32, 64, 128, 256]  # 128 do not modify it
        # layers = []
        # for i in range(len(self.mlp_dim) - 1):
        #     layers.append(linlayer(self.mlp_dim[i], self.mlp_dim[i + 1]))
        # self.mlp3 = nn.Sequential(*layers)
        # self.w_layernorm = nn.LayerNorm(256)   
    ...
    def forward(self, x):
        ...
        enc_output = self.attention_heads(enc_output, enc_output, enc_output)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        q = torch.stack([self.Q for _ in range(sz_b)], dim=1).view(-1, d_k)  # (N,T,d_model) 512, 6, 256

        k = self.fc1_k(v).view(sz_b, seq_len, n_head, d_k) # 512, 6, 16, 8
        k = k.permute(2, 0, 1, 3).contiguous().view(-1, seq_len, d_k)  # 8192, 6, 8

        v = torch.stack(v.split(v.shape[-1] // n_head, dim=-1)).view(n_head * sz_b, seq_len, -1) # 8192, 6, 16

        # weight = self.fc1_k(weight_load).view(sz_b, seq_len, n_head, d_k)
        # weight = weight.permute(2, 0, 1, 3).contiguous().view(-1, seq_len, d_k)
        output, attn = self.attention(q, k, v) #output 8192, 1, 6 attn 8192, 1, 6
