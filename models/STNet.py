...
class STNet(nn.Module):
    def __init__(self, input_dim=10, num_classes=9, d_model=128, n_head=16, n_layers=1, d_inner=128,
                 activation="relu", dropout=0.2, max_len=366, T=1000, max_temporal_shift=30):
...
if __name__ == '__main__':
    model = STNet()
    x = torch.randn(1, 10, 366)
    mask = torch.randint(0, 2, (1, 366)).bool()
    doy = torch.randint(0, 366, (1, 366))
    weight = torch.randn(1, 366)
    output = model((x, mask, doy, weight))
    print(output.shape)
    print(output)